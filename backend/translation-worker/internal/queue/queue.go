...
// TranslationJob represents a translation job message from RabbitMQ.
// This matches the TranslationJob structure from the server.
...
// Queue handles RabbitMQ operations for translation jobs.
type Queue struct {
	conn      *Connection
	queueName string
	exchange  string
...
// NewQueue creates a new translation queue.
func NewQueue(conn *Connection, queueName, exchange, routingKey string, prefetchCount int, logger *slog.Logger) (*Queue, error) {
	// Declare exchange
...
func (q *Queue) Consume(ctx context.Context, handler func(TranslationJob) error) error {
	...
	q.logger.Info("Started consuming translation jobs",
	...
	for {
		select {
		case <-ctx.Done():
			q.logger.Info("Stopping translation queue consumer")
			return ctx.Err()
		case msg, ok := <-msgs:
			if !ok {
			...
			if err := json.Unmarshal(msg.Body, &job); err != nil {
				q.logger.Error("Failed to parse translation job",
					slog.Any("error", err),
				)
			...
			if err := handler(job); err != nil {
				q.logger.Error("Failed to process translation job",
			...
			if err := msg.Ack(false); err != nil {
				...
				q.logger.Debug("Translation job processed successfully",
					slog.String("job_id", job.ID),
					slog.String("entity_type", job.EntityType),
					slog.String("entity_id", job.EntityID),
				)
