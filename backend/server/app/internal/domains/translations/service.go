...
func (s *service) fetchSourceTextFromEntity(ctx context.Context, entityType EntityType, entityID uuid.UUID, fields []string) (map[string]string, error) {
	...
	case EntityTypeSkill:
		...
		var skill Skill
		if err := s.db.WithContext(ctx).Table("skills").Where("id = ?", entityID).First(&skill).Error; err != nil {
			return nil, err
		}
		for _, field := range fields {
			switch field {
	...
	case EntityTypeSystemDesign:
		...
		if err := s.db.WithContext(ctx).Table("system_designs").Where("id = ?", entityID).First(&systemDesign).Error; err != nil {
			return nil, err
		}
		for _, field := range fields {
			switch field {
	...
	case EntityTypeProblemSolution:
		...
		if err := s.db.WithContext(ctx).Table("problem_solutions").Where("id = ?", entityID).First(&problemSolution).Error; err != nil {
			return nil, err
		}
		for _, field := range fields {
			switch field {
