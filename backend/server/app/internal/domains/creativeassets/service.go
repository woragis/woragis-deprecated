...
type service struct {
	repo             Repository
	creativeClient   *creativeservice.Client
}

// NewService creates a new service
func NewService(repo Repository, creativeClient *creativeservice.Client) Service {
	return &service{
		repo:           repo,
...
func (s *service) GenerateAndStoreThumbnail(ctx context.Context, userID uuid.UUID, entityType EntityType, entityID uuid.UUID, prompt, context string) (*CreativeAsset, error) {
	// Generate thumbnail using creative service
	resp, err := s.creativeClient.GenerateThumbnail(ctx, prompt, context)
	if err != nil {
		return nil, err
	}
	...
	// Store the asset
	asset := NewCreativeAsset(userID, entityType, entityID, AssetTypeImage, PurposeThumbnail)
	asset.SetImageData(resp.Data[0].B64JSON, resp.Data[0].URL, 0, 0, 0)
	asset.SetMetadata(prompt, string(resp.Provider), "png")
...
func (s *service) GenerateAndStoreDiagram(ctx context.Context, userID uuid.UUID, entityType EntityType, entityID uuid.UUID, description, diagramKind string) (*CreativeAsset, error) {
	// Generate diagram using creative service
	req := creativeservice.DiagramGenerationRequest{
		Description:  description,
		DiagramType:  creativeservice.DiagramTypeMermaid,
		DiagramKind:  diagramKind,
		OutputFormat: "png",
		AIProvider:   "openai",
	}

	resp, err := s.creativeClient.GenerateDiagram(ctx, req)
	if err != nil {
		return nil, err
	}

	// Store the asset
	asset := NewCreativeAsset(userID, entityType, entityID, AssetTypeDiagram, PurposeDiagram)
	asset.SetImageData(resp.B64JSON, "", 0, 0, 0)
	asset.SetDiagramData(resp.Code, resp.DiagramType)
	asset.SetMetadata(description, "mermaid", resp.Format)
