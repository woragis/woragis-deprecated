...
import (
	...
	"woragis-skills-service/internal/config"
	"woragis-skills-service/internal/database"
	"woragis-skills-service/pkg/health"
	applogger "woragis-skills-service/pkg/logger"
	appmetrics "woragis-skills-service/pkg/metrics"
	apptracing "woragis-skills-service/pkg/tracing"
	appsecurity "woragis-skills-service/pkg/security"
	apptimeout "woragis-skills-service/pkg/timeout"
	
	skillsdomain "woragis-skills-service/internal/domains"
)
...
func main() {
	...
	// Run migrations
	if err := skillsdomain.MigrateSkillsTables(dbManager.GetPostgres()); err != nil {
		slogLogger.Error("failed to run skills migrations", "error", err)
		os.Exit(1)
	}
	...
	// Setup skills domain routes
	// TODO: Initialize skills and interests domains
	// For now, placeholder setup - will be replaced with actual domains
	skillsdomain.SetupRoutes(api, dbManager.GetPostgres(), authServiceURL, slogLogger)
	...
	go func() {
		...
		slogLogger.Info("starting skills service", "addr", addr, "env", env)
	...
	<-ctx.Done()
	slogLogger.Info("shutting down skills service gracefully")
	...
	slogLogger.Info("skills service stopped")
}
