...
import (
	...
	"woragis-files-service/internal/config"
	"woragis-files-service/internal/database"
	"woragis-files-service/pkg/health"
	applogger "woragis-files-service/pkg/logger"
	appmetrics "woragis-files-service/pkg/metrics"
	apptracing "woragis-files-service/pkg/tracing"
	appsecurity "woragis-files-service/pkg/security"
	apptimeout "woragis-files-service/pkg/timeout"
	
	filesdomain "woragis-files-service/internal/domains"
)
...
func main() {
	...
	// Run migrations
	if err := filesdomain.MigrateFilesTables(dbManager.GetPostgres()); err != nil {
		slogLogger.Error("failed to run files migrations", "error", err)
		os.Exit(1)
	}
	...
	if storageBasePath == "" {
		storageBasePath = "/storage/files"
	}

	// Setup files domain routes
	filesdomain.SetupRoutes(api, dbManager.GetPostgres(), authServiceURL, storageBasePath, slogLogger)
	...
	go func() {
		...
		slogLogger.Info("starting files service", "addr", addr, "env", env)
	...
	<-ctx.Done()
	slogLogger.Info("shutting down files service gracefully")
	...
	slogLogger.Info("files service stopped")
}
