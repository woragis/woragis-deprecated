# Email Worker Integration Tests

This directory contains integration tests for the email worker service.

## Prerequisites

1. **Docker and Docker Compose** - Required to run test dependencies
2. **RabbitMQ** - Message queue for email processing

## Running Tests

### Using Docker Compose (Recommended)

From the `backend/server` directory:

```bash
# Start test dependencies
docker-compose -f docker-compose.test.yml up -d

# Run tests
cd ../../email-worker
go test -tags=integration ./internal/integration/...

# Cleanup
cd ../server
docker-compose -f docker-compose.test.yml down
```

### Manual Setup

1. Start RabbitMQ locally:
   ```bash
   docker run -d --name test-rabbitmq \
     -p 5673:5672 \
     -e RABBITMQ_DEFAULT_USER=test \
     -e RABBITMQ_DEFAULT_PASS=test \
     -e RABBITMQ_DEFAULT_VHOST=/test \
     rabbitmq:3.13-management-alpine
   ```

2. Set environment variables:
   ```bash
   export RABBITMQ_URL="amqp://test:test@localhost:5673/test"
   ```

3. Run tests:
   ```bash
   go test -tags=integration ./internal/integration/...
   ```

## Test Coverage

### Queue Setup Tests
- ✅ `TestEmailWorkerQueueSetup` - Tests queue and exchange creation
- ✅ `TestEmailWorkerMessagePublish` - Tests publishing messages to queue

### Message Processing Tests
- ✅ `TestEmailWorkerMessageConsume` - Tests consuming and processing messages
- ✅ `TestEmailWorkerInvalidMessage` - Tests handling of invalid JSON messages
- ✅ `TestEmailWorkerRetryOnFailure` - Tests retry behavior on send failure
- ✅ `TestEmailWorkerMultipleMessages` - Tests processing multiple messages

### Dead Letter Queue Tests
- ✅ `TestEmailWorkerDeadLetterQueue` - Tests DLQ setup and binding

## Test Structure

Tests use a mock email sender to avoid actually sending emails during testing. The mock sender:
- Tracks sent messages
- Can simulate failures for retry testing
- Validates message content

## Environment Variables

- `RABBITMQ_URL` - RabbitMQ connection URL (default: `amqp://test:test@localhost:5673/test`)
