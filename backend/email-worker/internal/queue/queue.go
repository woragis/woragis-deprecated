...
// EmailEnvelope represents the email message payload from RabbitMQ.
...
// Queue handles RabbitMQ operations for email messages.
type Queue struct {
	conn      *Connection
	queueName string
	exchange  string
...
func (q *Queue) Consume(ctx context.Context, handler func(EmailEnvelope) error) error {
	...
	q.logger.Info("Started consuming email messages",
	...
	for {
		select {
		case <-ctx.Done():
			q.logger.Info("Stopping email queue consumer")
			return ctx.Err()
		case msg, ok := <-msgs:
			if !ok {
			...
			if err := json.Unmarshal(msg.Body, &envelope); err != nil {
				q.logger.Error("Failed to parse email message",
					slog.Any("error", err),
				)
			...
			if err := handler(envelope); err != nil {
				q.logger.Error("Failed to process email message",
			...
			if err := msg.Ack(false); err != nil {
				...
				q.logger.Debug("Email message processed successfully",
					slog.String("user_id", envelope.UserID),
					slog.String("destination", envelope.Destination),
				)
			}
