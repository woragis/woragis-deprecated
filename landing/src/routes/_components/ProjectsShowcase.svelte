						...
						<div class="mb-4">
							<div class="flex flex-wrap gap-2">
								{#each project.technologies.slice(0, 5) as tech}
									...
									<div
										class="flex items-center gap-1 px-2 py-1 bg-gray-700/50 rounded text-xs text-gray-300 border border-gray-600 hover:border-blue-500/50 transition-colors"
										title={tech.name}
									>
										{#if TechIcon}
					...
					<div class="flex items-center gap-2 mb-4 flex-wrap">
						...
						{#if project.technologies?.some((t) => t.link)}
							<a
								...
								onclick={(e) => e.stopPropagation()}
								class="flex items-center gap-1 px-3 py-1.5 bg-green-600/20 hover:bg-green-600/30 text-green-300 text-xs rounded border border-green-600/30 transition-colors"
							>
								<Globe class="w-3 h-3" />
								View Live
							</a>
						...
						{#if project.technologies?.some((t) => t.link?.includes('github'))}
							<a
								...
								rel="noopener noreferrer"
								onclick={(e) => e.stopPropagation()}
								class="flex items-center gap-1 px-3 py-1.5 bg-gray-700/50 hover:bg-gray-600/50 text-gray-300 text-xs rounded border border-gray-600 transition-colors"
							>
