<script lang="ts">
	...
	import { Plus, Briefcase, AlertCircle, Edit, Trash2, Globe } from 'lucide-svelte';
	import {
		listCaseStudies,
		createCaseStudy,
		updateCaseStudy,
	...
	let studies: CaseStudy[] = $state([]);
	let loading = $state(true);
	let error: string | null = $state(null);
	let showCreateModal = $state(false);
	let showEditModal = $state(false);
	let showTranslationModal = $state(false);
	let editingStudy: CaseStudy | null = $state(null);
	let translatingStudyId: string | null = $state(null);
	...
	function startEdit(study: CaseStudy) {
		editingStudy = study;
		formTitle = study.title;
		formContent = study.content;
		formExcerpt = study.excerpt || '';
		formFeatured = study.featured;
		showEditModal = true;
	}
	...
	async function handleSave() {
		...
		try {
			const payload: CreateCaseStudyInput = {
			...
			if (editingStudy) {
				await updateCaseStudy(editingStudy.id, payload);
			...
			toastError(err instanceof Error ? err.message : 'Failed to save case study');
		}
	}
	...
	async function handleDelete(id: string) {
		...
		try {
			...
			toastSuccess('Case study deleted successfully');
			await fetchStudies();
		} catch (err) {
			toastError(err instanceof Error ? err.message : 'Failed to delete case study');
		}
	}

	function startTranslation(studyId: string) {
		translatingStudyId = studyId;
	...
	function cancelTranslation() {
		...
		translatingStudyId = null;
	}

	async function handleRequestTranslation() {
		if (!translatingStudyId) return;
...
<div class="page-container">
	<div class="page-header">
		<div>
			...
			<p class="page-description">Manage project case studies</p>
		</div>
	...
	{:else if studies.length === 0}
		<div class="empty-state">
			...
			<p class="empty-title">No case studies found</p>
			<p class="empty-description">Create your first case study to get started</p>
		</div>
	{:else}
				...
				<tbody>
					{#each studies as study}
						<tr>
							...
							<td class="text-muted">{formatDate(study.created_at)}</td>
							<td class="text-right">
								<div class="actions">
									<button
										type="button"
										class="btn btn-sm btn-secondary"
										onclick={() => startTranslation(study.id)}
...
{#if showCreateModal || showEditModal}
	<div class="modal-overlay" onclick={cancelEdit}>
		<div class="modal modal-large" onclick={(e) => e.stopPropagation()}>
			<h2 class="modal-title">{editingStudy ? 'Edit Case Study' : 'Create Case Study'}</h2>
			<div class="modal-content">
				<div class="form-group">
					<label class="form-label">Title *</label>
					<input type="text" bind:value={formTitle} class="input" placeholder="Case study title" />
				</div>
				<div class="form-group">
...
<!-- Translation Modal (reuse from posts) -->
{#if showTranslationModal && translatingStudyId}
