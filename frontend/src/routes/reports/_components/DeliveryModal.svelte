<script lang="ts">
	import type { DeliveryFormState } from '../reports.logic';

	export let open = false;
	export let mode: 'create' | 'edit' = 'create';
	export let form: DeliveryFormState;
	export let onFieldChange: <K extends keyof DeliveryFormState>(
		field: K,
			...
			<div class="space-y-3 text-sm text-slate-100">
				<label class="text-xs font-medium uppercase tracking-wide text-slate-400">
					...
					<input
						...
						value={form.channel}
						on:input={(event) => onFieldChange('channel', (event.target as HTMLInputElement).value)}
					/>
				</label>
				<label class="text-xs font-medium uppercase tracking-wide text-slate-400">
					Target
					<input
						class="mt-1 w-full rounded-lg border border-slate-700 bg-slate-900/80 px-3 py-2 text-sm text-slate-100 focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary/40"
						value={form.target}
						on:input={(event) => onFieldChange('target', (event.target as HTMLInputElement).value)}
					/>
				</label>
				<label class="text-xs font-medium uppercase tracking-wide text-slate-400">
					Template (JSON)
					<textarea
						...
						on:input={(event) => onFieldChange('templateText', (event.target as HTMLTextAreaElement).value)}
					></textarea>
				</label>
