<script lang="ts">
	...
	async function loadDefaults() {
		...
		// If localStorage is empty, user can enter website manually
	}
	...
	function loadTemplate(template: ApplicationTemplate) {
		if (template.location) {
			formLocation = template.location;
		}
		if (template.website) {
			formWebsite = template.website;
			loadResumeForWebsite(template.website);
		}
		if (template.interestLevel) {
			formInterestLevel = template.interestLevel;
		}
		...
		if (template.coverLetter) {
			formCoverLetter = template.coverLetter;
		}
		if (template.status) {
